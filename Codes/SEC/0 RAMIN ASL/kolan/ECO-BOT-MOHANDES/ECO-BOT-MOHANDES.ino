...
float tsop_cal[16] = { // tarif yek araye ye 16 taii baraye eslah maghadir TSOP
  1.0000,  //  0
  1.0482,  //  1
  1.0056,  //  2
  1.0373,  //  3
...
void movexy(float vx, float vy) {
  // MOHASEBE MEGHDAR HAR CHARKH
  float ml1_move =  vy + vx;
  float ml2_move =  vy - vx;
  float mr1_move = -vy + vx;
  float mr2_move = -vy - vx;
  // MOHASEBE GHADR MOTLAGH BARAYE PEIDA KARDAN GHODRAT KHALES HAR CHARKH
  float powers[4] = { abs(ml1_move), abs(ml2_move), abs(mr1_move), abs(mr2_move) };
  float vals[4]   = {     ml1_move,      ml2_move,      mr1_move,      mr2_move  };
 // TABEII BARAYE MORATAB SAZI CHARKH HA AZ BOZORG BE KOCHIK
  int idx[4] = {0, 1, 2, 3};
  for (int i = 0; i < 3; i++) 
    for (int j = i + 1; j < 4; j++)
      if (powers[idx[i]] < powers[idx[j]]) { // AGHA SINA IN NIAZI BE ELSE NADARE BARA ESBATESH MITONI ADAD RO JA GOZARI KONI
        int tmp = idx[i]; idx[i] = idx[j]; idx[j] = tmp;
      }

  float d_move[4] = {0, 0, 0, 0}; // JA GOZARIE BADAD BE DAST OMADE
  d_move[idx[0]] = vals[idx[0]];
  d_move[idx[1]] = vals[idx[1]];

  motor(d_move[0], d_move[1], d_move[2], d_move[3]);
}
void sensor() {
...
void out() {
  ...
  {
    if (shl > 1900) movesecond(120, out_sec * 1.3);
