...
void sensor() {
  ...
  for (int i = 0 ; i < 16; i++) {
    ...
    // if ( i == 15) value = value * 0.8 ;
    // if (i == 1 || i == 2) value = value * 0.8 ;
  ...
  if (shr > 1400) dgy = 25;
  else if (shl > 1400) dgy = -25;
  else if (!isball) dgy = 0;
}
...
void out() {
  ...
  {
    movesecond(-90, 10);
    while (ballangle > 0 && isball && cnt < 20)
    {
      sensor();
      printall();
      stop();
  ...
  /////BACK
    ...
    movesecond(0, 17);
    while ((ballangle >= 90 || ballangle <= -90) && isball && cnt < 20)
    {
      sensor();
      printall();
      stop();
...
void loop() {
  ...
  if (ball_in_kicker) {
    ...
    out();
    float shift = clamp(ballangle * 0.6, -60, 60 );
