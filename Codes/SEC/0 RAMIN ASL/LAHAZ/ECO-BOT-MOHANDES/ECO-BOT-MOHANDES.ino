...
void setup() {
  ...
  display.print("loading... ");  // loding startup
...
void loop() {  // halghe tekrar bara hamishe
  ...
  if (ball_in_kicker && isball) {  // age ham sensor dahane ham tsop ha top ro tashkhis dadan
    ...
    v = 55000; // niazi be map nist chon in dare ro 2 ta charkh kar mikone enghadi sari nist ke moshkelsaz she
    // v = map(balldistance, 10000, 3000, 30000, 50000);  // tabdile sorat bar asas fasele ye top
